...
// Effect: Consume a sequence of rowsets work from a queue, creating a fractal tree.  Closes fd.
{
    ...
    while (result == 0) {
        ...
        for (unsigned int i = 0; i < output_rowset->n_rows; i++) {
            ...
            add_pair_to_leafnode(lbuf, (unsigned char *) key.data, key.size,
                                 (unsigned char *) val.data, val.size,
                                 this_leafentry_size, &deltas);
            n_rows_remaining--;
    ...
    while (sts->n_subtrees > 1) {
        ...
        while (sts->n_subtrees - n_subtrees_used >= n_per_block*2) {
            ...
            if (r) {
                ...
                // frees all the data structures that go into making the node.
                write_nonleaf_node(bl, out, blocknum_of_new_node,
                                   n_per_block, pivots, subtree_info,
                                   height, descriptor);
        ...
        if (result == 0) {
            ...
            if (n_blocks_left > n_per_block) {
                ...
                if (r) {
                    ...
                    write_nonleaf_node(bl, out, blocknum_of_new_node,
                                       n_first, pivots, subtree_info, height,
                                       descriptor);
        ...
        if (result == 0) {
            ...
            if (r) {
                ...
                write_nonleaf_node(bl, out, blocknum_of_new_node,
                                   n_blocks_left, pivots, subtree_info,
                                   height, descriptor);
